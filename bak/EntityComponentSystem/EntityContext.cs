...
using Alien_World.Entity_Component_System.Group;
...
namespace Alien_World.Entity_Component_System
{
    ...
    public class EntityContext : IDisposable
    {
        ...
        readonly Dictionary<ComponentMask, EntityGroup> m_Groups = new Dictionary<ComponentMask, EntityGroup>();
        readonly List<EntityGroup>[] m_GroupsForIndex = new List<EntityGroup>[64];
        readonly ObjectPool<List<GroupChanged>> m_GroupChangedListPool
            = new ObjectPool<List<GroupChanged>>(() => new List<GroupChanged>());
        ...
        public Entity Create()
        {
            ...
            Entity result = new Entity(this, new Entity.Id(index, version));
            result.OnComponentAdded += m_CachedEntityChanged;
            result.OnComponentRemoved += m_CachedEntityChanged;
            return result;
        }
        ...
        internal int Remove<C>(Entity.Id id) where C : class, IComponent, new()
        {
        ...
        internal int Assign<C>(Entity.Id id, C c) where C : class, IComponent, new()
        {
            AssertValidID(id);
            int family = ComponentIndexer.GetFamily<C>();
            int entityId = (int)id.Index;
